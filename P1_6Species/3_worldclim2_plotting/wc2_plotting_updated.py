...
Plots the quantity of all 19 variables for each species and saves them
'''
...
index_WC2_var = 0 #used for naming and knowing what wc2 variable we are on

#________________Loop Through Files - WC2______________________
for i in range(len(WC2_list)):
	#load specific file
	filename = os.fsdecode(WC2_list[i])
	...
	for file2 in os.listdir(Data_dir):
		...
		for index, row in data_.iterrows():
		    ...
		    if int(raster_pix) < num1:
		    	data_.at[index, 'raster_pixel'] = float('NAN');
		    else:
